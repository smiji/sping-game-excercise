...
class SpinBetMultiplayerMultiPocketResolverTest {

    private static final String FILE_MULTI_PLAYER_MULTI_POCKET = "/player/MultiplayerMultipocket.json";

    private static final String FILE_MULTI_PLAYER_MULTI_POCKET_FORCED_WIN = "/player/MultiplayerMultipocket_forced.json";
    ...
    public void setUp() {
        ...
        betResolver = Map.of(BetMode.MULTI_PLAYER_MULTI_POCKET.name(), new SpinBetMultiplayerMultiPocketResolver());
        rouletteService = new RouletteService(rouletteWheel, betResolver);
    }

    @Test
    void testSpinResult_multi_player() throws IOException {
        SpinResultDTO spinResult = rouletteService.spin(Util.deserialize(SpinDTO.class, Util.readJsonFromFile(FILE_MULTI_PLAYER_MULTI_POCKET)));
    ...
    void testSpinResult_multi_player_forced_win() throws IOException {
        SpinResultDTO spinResult = rouletteService.spin(Util.deserialize(SpinDTO.class, Util.readJsonFromFile(FILE_MULTI_PLAYER_MULTI_POCKET_FORCED_WIN)));
        String namesOfParticipants = spinResult.getBetResults().stream().map(betResultDTO -> betResultDTO.getPlayerBet().getPlayerName()).collect(Collectors.joining(","));
        ...
        assertEquals(0, julesWinAmount.compareTo(BigDecimal.valueOf(360L)), "Jules expected to have 72 win amount");
    }
